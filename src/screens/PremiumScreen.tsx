...
export function PremiumScreen(): React.JSX.Element {
  ...
  const perks = [
    ...
    { icon: <SparkIcon size={18} />, title: 'Exclusive feeds', copy: 'Premium-only lists and early access to new features.' },
  ...
  return (
    <section className="screen screen--premium">
      ...
      <div className="premium-hero">
        ...
        <div>
          <p className="eyebrow">Member exclusives</p>
          <h2>X-sutra Premium</h2>
          <p>Exclusive clips posted by admin. Watch them here inside the app.</p>
        </div>
      </div>
      ...
      {posts.length > 0 && (
        <>
          <div className="section-heading section-heading--spaced"><div><p className="eyebrow">Drops</p><h3>Premium clips</h3></div><span>{posts.length} clips</span></div>
      ...
      >
        {account ? `You're signed in as ${account.name}` : 'Sign in'}
      </button>
    </section>
  )
}
