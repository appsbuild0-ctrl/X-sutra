...
export const handler = async (event) => {
  ...
  try {
    ...
    const { password, filename, contentType, data, content, channelId } = body

    // Require admin authorization — either the bearer token or the admin
    // password in the body, which is what the admin console sends.
    ...
    // Upload to Discord
    // The admin picks the target channel in the console; without one the
    // channel configured as DISCORD_CHANNEL_ID is used.
    const result = await uploadToDiscord(buffer, safeName, mime, content || '', channelId)
