...
export function NicheScreen(): React.JSX.Element {
  ...
  return (
    <section className="screen">
      ...
      {feed.error && feed.items.length === 0 ? <LiveError message={feed.error} onRetry={feed.reload} title="Niche clips could not load." /> : <MediaGrid items={feed.items} loading={feed.loading} canLoadMore={feed.canLoadMore} loadingMore={feed.loadingMore} onLoadMore={() => void feed.loadMore()} empty={<div className="empty-state"><strong>No public clips are available in this niche.</strong></div>} />}
